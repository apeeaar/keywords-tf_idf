...
for i in range(len(pol)):
    local=(d[str(pol[i])]) * (idfs[str(pol[i])])
    tf_idf.append(local)
final_dic=dict(zip(pol,tf_idf))
d_sorted_by_value = OrderedDict(sorted(final_dic.items(), key=lambda x: x[1]))
keywords=list(d_sorted_by_value)
print(keywords[0:5])
